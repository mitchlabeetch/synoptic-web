  ...
  return (
    ...
    >
      ...
      {showHeader && (
        ...
        >
          <span className="truncate max-w-[40%]">{page.headerText || meta?.title || t('drafting')}</span>
        ...
        {page.blocks.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-muted/20 rounded-2xl bg-muted/5 group hover:bg-muted/10 transition-colors">
            <div className="h-12 w-12 rounded-full bg-muted/20 flex items-center justify-center mb-4 text-muted-foreground/40 group-hover:scale-110 transition-transform">
               <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-file-plus-2"><path d="M4 22h14a2 2 0 0 0 2-2V7l-5-5H6a2 2 0 0 0-2 2v4"/><path d="M3 15h6"/><path d="M6 12v6"/></svg>
            </div>
            <p className="text-sm text-muted-foreground italic font-medium">{t('ready')}</p>
          </div>
        )}
      </div>
    </div>
