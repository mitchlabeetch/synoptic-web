...
export function CommandPalette() {
  ...
  const actions = [
    ...
    },
    {
      ...
      icon: Download,
      action: () => {
        // Find existing publish button and click it to switch tab if not simplified
  ...
  return (
    <Command.Dialog 
      ...
      label="Command Menu"
      className="fixed inset-0 z-[100] flex items-start justify-center pt-[20vh] bg-background/50 backdrop-blur-sm animate-in fade-in duration-300 px-4"
    >
      <div className="w-full max-w-[600px] bg-card border rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <Command.List className="max-h-[350px] overflow-auto p-2">
          ...
          <Command.Group heading="Actions" className="px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">
            {actions.map((action) => (
              <Command.Item
                key={action.id}
                onSelect={() => {
        ...
        <div className="bg-muted/30 p-3 border-t flex justify-between items-center px-6">
           <div className="flex items-center gap-4">
             <div className="flex items-center gap-1">
               <span className="text-[10px] lowercase text-muted-foreground font-medium">select</span>
             ...
             <div className="flex items-center gap-1">
               <span className="text-[10px] lowercase text-muted-foreground font-medium">navigate</span>
               <div className="px-1.5 py-0.5 rounded border bg-background text-[9px] font-black tracking-tighter shadow-sm text-muted-foreground">↑↓</div>
             </div>
           </div>
           
           <div className="text-[10px] text-muted-foreground/60 font-bold uppercase tracking-wider">
             Synoptic Command Palette
           </div>
        </div>
      </div>
    </Command.Dialog>
